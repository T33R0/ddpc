...
| **user_vehicle** | User's personal vehicle collection | id (PK, UUID), owner_id (FK → auth.users), vin, year, make, model, trim, nickname, title, privacy (PRIVATE/PUBLIC), photo_url, stock_data_id (FK → vehicle_data), spec_snapshot (JSONB), current_status (active/inactive/archived), odometer, horsepower_hp, torque_ft_lbs, engine_size_l, cylinders, fuel_type, drive_type, transmission, body_type, colors_exterior, epa_combined_mpg, length_in, width_in, height_in | owner_id → auth.users(id), stock_data_id → vehicle_data(id) | [✅ CHECKED] owner_id = auth.uid() for ALL ops, public SELECT for privacy='public' |
