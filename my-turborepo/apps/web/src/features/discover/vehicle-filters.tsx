...
export function VehicleFilters({ filters, onFilterChange, vehicles }: VehicleFiltersProps) {
  ...
  const allTrims = vehicles.flatMap(v => v.trims);
  const engineTypes = Array.from(new Set(allTrims.map(t => t.cylinders).filter(Boolean))).sort() as string[];
  const fuelTypes = Array.from(new Set(allTrims.map(t => t.fuelType).filter(Boolean))).sort() as string[];
  const drivetrains = Array.from(new Set(allTrims.map(t => t.driveType).filter(Boolean))).sort() as string[];
  const doors = Array.from(new Set(allTrims.map(t => t.doors).filter(Boolean))).sort() as string[];
  const vehicleTypes = Array.from(new Set(allTrims.map(t => t.bodyType).filter(Boolean))).sort() as string[];
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 mb-8">
      <div className="flex flex-wrap items-center gap-4">
        ...
        <DropdownMenu options={[{ label: 'All', onClick: () => handleValueChange('doors')(null) }, ...doors.map(d => ({ label: d ?? '', onClick: () => handleValueChange('doors')(d) }))]} >{filters.doors || 'Doors'}</DropdownMenu>
